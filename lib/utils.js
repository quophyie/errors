...
const tryRequestPromiseErrorConversion = err => {
  ...
  const isETypeErr = ['ECONNREFUSED', 'EADDRINUSE', 'ECONNRESET', 'ETIMEDOUT', 'EPIPE'].find((code) => (err.code && code) && (err.code.trim().toLowerCase() === code.trim().toLowerCase())) !== undefined
  if (err instanceof RequestPromiseErrors.RequestError || err.name === 'RequestError' || isETypeErr) {
...
module.exports = {
     ...
     */
  toBoom: (err, customErrors, customMapping, logger) => {
    ...
    if (!boomError) {
      switch (err.name) {
        ...
        case 'ValidationError' : {
          if (err.isJoi && !err.isBoom) { boomError = Boom.badRequest(err) } else { boomError = Boom.wrap(err) }
          break
        }
        default: {
          boomError = Boom.wrap(err) // defaults to HTTP 500 status
        }
      }
    }
    ...
    if (logger) {
      logger.error(boomError, boomError.message)
    }
    return boomError
  },

    /**
     * Convert bookshelf error to standard Common Application error
     * @param {Error} bookshelfErr
     * @return {Error}
     */
  bookshelfToApp: bookshelfErr => {
